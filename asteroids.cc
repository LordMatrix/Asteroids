...
void drawButton(tButton button) {
	...
	MOMOS::DrawSetTextSize(font);

	const char *txt2 = button.txt.c_str();
	MOMOS::DrawText(button.pos.x + padding, button.pos.y + padding * 2, txt2);
}
...
int checkButtonsClick() {
	...
	for (i = 0; i < num_buttons && !click; i++) {
		...
		if (MOMOS::MousePositionX() > (int)buttons[i].pos.x
			&& MOMOS::MousePositionX() < (int)buttons[i].pos.x + 2 * buttons[i].size
			&& MOMOS::MousePositionY() > (int)buttons[i].pos.y
			&& MOMOS::MousePositionY() < (int)buttons[i].pos.y + buttons[i].size) {
			click = 1;
		}
	}
	return (click) ? i - 1 : -1;
...
void printFigure(tFigura figura) {
	...
	/*Color rgb interior del polígono*/
	MOMOS::DrawSetStrokeColor(figura.rgb[0], figura.rgb[1], figura.rgb[2], figura.rgb[3]);
	//MOMOS::DrawSetFillColor(0, 0, 0, 0);
	MOMOS::DrawSetFillColor(figura.rgb[0], figura.rgb[1], figura.rgb[2], figura.rgb[3]);

	/*Pinta la misma figura rellena. El último parámetro determina si se muestra el borde*/
	MOMOS::DrawPath(pathPoints, figura.num_vertices + 1);
}
...
void moveShip(tShip(*ship)) {
	...
	if (MOMOS::IsKeyPressed('A')) {
		rotateFigure(&(*ship).figura, degrees);
		(*ship).dir -= degrees;
	}
	//D -> Rotar a la derecha
	else if (MOMOS::IsKeyPressed('D')) {
		rotateFigure(&(*ship).figura, -degrees);
		(*ship).dir += degrees;
	}

	//W -> Avanzar
	if (MOMOS::IsKeyPressed('W')) {
...
void moveBlasts() {
	...
	for (j = 0; j < num_blasts; j++) {
		for (i = 0; i < PARTICLES_PER_BLAST; i++) {
			...
			MOMOS::DrawLine(p.x, p.y, p.x + 1, p.y + 1);
		}
...
void initText() {
	...
	MOMOS::DrawSetFillColor(255, 255, 255);
...
void printInfo() {
	...
	MOMOS::DrawText(100.0f, 30.0f, str);
	//Nivel
	MOMOS::DrawText(350.0f, 30.0f, "Lvl. ");
	_itoa_s(level, str, 10);
	MOMOS::DrawText(470.0f, 30.0f, str);
	//Vidas
...
void showFPS(float diff) {
	...
	MOMOS::DrawText(650.0f, 30.0f, buf);
}
...
void lightBox(char txt[]) {
	...
	MOMOS::DrawText(x + padding, y + padding * 2, txt);


	while (MOMOS::WindowIsOpened() && i < ttl) {
		...
		MOMOS::DrawSetFillColor(fontAlpha, fontAlpha, fontAlpha, 255);
		MOMOS::DrawSetStrokeColor(fontAlpha, fontAlpha, fontAlpha, 255);
		MOMOS::DrawText(x + padding, y + padding * 2, txt);
		i++;
...
int game() {
	...
	while (MOMOS::WindowIsOpened() && quit == 0) {
		...
		if (MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Enter) || MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Escape)) {
			initPauseMenu();
			pause = !pause;
		}

		if (pause && player.lives > 0) {
			MOMOS::DrawSetTextSize(70);
			MOMOS::DrawText(300.0f, 160.0f, "PAUSED");
		...
		if (player.lives < 1) {
			...
			if (game_over_age < 400) {
				MOMOS::DrawSetTextSize(50);
				MOMOS::DrawText(win_width / 3, win_height / 2, "GAME OVER");
				game_over_age++;
			}
			else {
		...
		case 0:
			...
			if (!pause) {
				...
				if (MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Space) && num_shots < 5) {
...
int checkTextBoxesClick() {
	...
	for (i = 0; i < num_textBoxes && !click; i++) {
		//Comprobamos que el click sea en el interior del botón
		if (MOMOS::MousePositionX() > (int)textBoxes[i].pos.x
			&& MOMOS::MousePositionX() < (int)textBoxes[i].pos.x + 8 * textBoxes[i].size
			&& MOMOS::MousePositionY() > (int)textBoxes[i].pos.y
			&& MOMOS::MousePositionY() < (int)textBoxes[i].pos.y + textBoxes[i].size) {
			click = 1;
		}
	}
	return (click) ? i - 1 : -1;
...
void printTextBox(tTextBox box, int active) {
	...
	MOMOS::DrawText(box.pos.x, box.pos.y + 30, box.txt);
}
...
void textEditor(Point2 pos, char str[50], int *length) {
	...
	key = MOMOS::GetNextPressedKey();
	if (key > 0) {
		if (MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Enter)) {
			quit = 1;
		}
		else if (MOMOS::IsKeyDown(8) && *length > 0) {
			str[*length - 1] = '\0';
			*length -= 1;
		}
...
void printTextBoxes(int active_box) {
	...
	for (int i = 0; i < num_textBoxes; i++) {
		...
		MOMOS::DrawText(130.0f, textBoxes[i].pos.y + 30, title);
...
void highScoresMenu() {
	...
	while (MOMOS::WindowIsOpened() && !quit) {
		...
		MOMOS::DrawText(300.0f, 50.0f, "HIGHSCORES");
		...
		for (i = 0; i < num_highScores && i < 10; i++) {
			...
			if (currentPlayer) {
				...
				MOMOS::DrawSetStrokeColor(50, 250, 50);
				MOMOS::DrawSetFillColor(50, 250, 50);
			}

			//Top
			_snprintf_s(top_str, 30, "#%d", i + 1);
			MOMOS::DrawText(x, y, top_str);
			x += 100;
			//Username
			MOMOS::DrawText(x, y, highScores[i].user);
			x += 200;
			//Date
			localtime_s(&timeinfo, &highScores[i].date);
			strftime(date_str, 80, "%d-%m-%Y", &timeinfo);
			MOMOS::DrawText(x, y, date_str);
			x += 350;
			//Score
			_itoa_s(highScores[i].score, score_str, 10);
			MOMOS::DrawText(x, y, score_str);
			y += 30;
...
void LoggedInMenu() {
	...
	while (MOMOS::WindowIsOpened() && quit == 0) {
		...
		MOMOS::DrawSetStrokeColor(255, 255, 255);
		MOMOS::DrawSetTextSize(20);
		MOMOS::DrawText(100.0f, 20.0f, user_str);
		MOMOS::DrawText(650.0f, 20.0f, credits_str);

		MOMOS::DrawSetTextSize(100);
		MOMOS::DrawText(130.0f, 160.0f, "ASTEROIDS");
...
void logInMenu(int option) {
	...
	while (MOMOS::WindowIsOpened() && quit == 0) {
		...
		if (MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Tab) && active_box < num_textBoxes) {
			fflush(stdin);
			active_box++;
		}
		if (active_box >= 0) {
		...
		if (!MOMOS::IsSpecialKeyDown(MOMOS::kSpecialKey_Escape)) {
			if (option == 0) {
				MOMOS::DrawText(130.0f, 50.0f, "LOG IN");
			}
			else {
				MOMOS::DrawText(130.0f, 50.0f, "REGISTER");
			}
		}
		else
			quit = 1;
